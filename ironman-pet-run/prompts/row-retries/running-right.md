Create Codex pet row `running-right` for `ironman`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Iron Man-inspired compact armored superhero mascot: glossy red-and-gold sci-fi armor, cyan circular chest reactor and narrow glowing cyan eye slits, friendly confident expression, chibi 3D toy style, no text, logos, weaponry, or trademarks.. Preserve silhouette, face, palette, material, proportions, markings, and props.
